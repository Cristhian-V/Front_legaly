...
const Expedientes = () => {
  ...
  const getEstadoDisplay = (estado) => estado === 'Cerrado' ? 'Pasivo' : estado;
  ...
  return (
    <main className="flex-1 overflow-x-hidden overflow-y-auto px-4 py-4 md:px-8 md:py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <button onClick={() => setIsModalOpen(true)} className="w-full md:w-auto bg-[#1E3A5F] hover:bg-slate-700 text-white font-semibold py-2 px-6 rounded-lg shadow-md transition-colors flex items-center justify-center gap-2">
                ...
                {casosFiltrados.length > 0 ? (
                  casosFiltrados.map((caso, index) => (
                    <tr key={index} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4 text-center"><span className={`inline-block px-3 py-1.5 rounded-xl text-xs font-bold border text-center leading-tight shadow-sm ${getEstadoBadge(caso.estado_nombre)}`}>{getEstadoDisplay(caso.estado_nombre)}</span></td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 backdrop-blur-md p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden animate-fade-in-up flex flex-col max-h-[90vh]">
            ...
            <div className="bg-[#152844] px-6 py-4 flex justify-between items-center flex-shrink-0">
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto">
              ...
              <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-gray-100 flex-shrink-0">
                ...
                <button type="submit" disabled={guardando} className="px-6 py-2.5 text-sm bg-[#152844] hover:bg-slate-800 text-white font-bold rounded-lg shadow-md transition disabled:bg-gray-400 flex items-center gap-2">
                  {guardando ? (
                    <>
